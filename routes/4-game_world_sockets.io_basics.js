        ...
        loopCycleEvents: function() {
            if(tickCount === 0) {
                serverCycleTickInterval = setInterval( () => {
                    ...
                    Server.GameEventsEmitter.emit("newCycleBegin", "A New Cycle Begins");
...
//
// HOST SETUP AND INIT
let gameData = hostCreateNewGame();
...
Server.bindHostEvents = function(socket) {
    ...
    socket.on('join', function(data) {
        ...
        socket.emit('joinedClientId', Server.clientIdsCounter);
        console.log('a new user with id ' + Server.clientIdsCounter + " has entered with message: " + data + " and id: " + this.id);
        Server.clientIds.push({ id:Server.clientIdsCounter, socketId: socket.id });        
    });
    socket.on('fetchGameId', function(data) {
        ...
        socket.emit('fetchGameIdResponse', gameData);
    });
    ...
    socket.on('onBeginNewGame', function(msg, roomId) {

    });
    socket.on('onRefreshBrokerage', function() {
